...
class TetrisPlayer3:
    ...
    def _cost_fill_rows(self, state, height, shape_height):
        ...
        cost = 0
        for row in range(height, max_height):
    ...
    def _cost_combined(self, state, height, shape_height):
        """
        Calcula el costo del futuro dado, en función del tamaño de las
        filas que coinciden con la posición del agente y la altura.
        """
        max_height = height + shape_height
        cost = 0
        for row in range(height, max_height):
        ...
        killed_rows = self._kill_rows(state, max_height)
        cost -= killed_rows * 20
        cost += self._count_holes(state, max_height) * 5 + max_height * 10
        return state, cost
